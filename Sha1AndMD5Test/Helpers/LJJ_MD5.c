...
//MD5算法
//每次处理的BLOCK的大小
static const size_t LJJ_MD5_BLOCK_SIZE = 64;
 ...
 */
static void ljj_md5_process_block(uint32_t state[4], const uint32_t block[LJJ_MD5_BLOCK_SIZE / 4])
{
    ...
    x = block;
#else
    uint32_t swap_block[LJJ_MD5_BLOCK_SIZE / 4];
 ...
 */
static void ljj_md5_update(md5_ctx *ctx, const unsigned char *buf, size_t size)
{
    //为什么不是=，因为在某些环境下，可以多次调用zen_md5_update，但这种情况，必须保证前面的调用，每次都没有unprocessed_
    ctx->length_ += size;
    //每个处理的块都是64字节
    while (size >= LJJ_MD5_BLOCK_SIZE)
    {
        ljj_md5_process_block(ctx->hash_, (const uint32_t *)(buf));
        buf  += LJJ_MD5_BLOCK_SIZE;
        size -= LJJ_MD5_BLOCK_SIZE;
    }
 ...
 */
static void ljj_md5_final(md5_ctx *ctx, const unsigned char *buf, size_t size, unsigned char *result)
{
    uint32_t message[LJJ_MD5_BLOCK_SIZE / 4];
    ...
    if (index > 14)
    {
        ...
        ljj_md5_process_block(ctx->hash_, message);
        index = 0;
    }
    //补0
    while (index < 14)
...
#if LJJ_BYTES_ORDER == LJJ_LITTLE_ENDIAN
    memcpy(result, &ctx->hash_, LJJ_MD5_HASH_SIZE);
#else
    swap_uint32_memcpy(result, &ctx->hash_, LJJ_MD5_HASH_SIZE);
#endif
...
unsigned char *LJJ_MD5(const char *buf,
                       size_t size) {
    unsigned char result[LJJ_MD5_HASH_SIZE + 1] = {0};
    md5_ctx ctx;
    ljj_md5_init(&ctx);
    ljj_md5_update(&ctx, (const unsigned char *)buf, size);
    ljj_md5_final(&ctx, (const unsigned char *)buf, size, result);
    result[LJJ_MD5_HASH_SIZE] = '\0';
    size_t crypt_len = 0;
    unsigned char *base64Char = (unsigned char *)base64_encode(result, LJJ_MD5_HASH_SIZE, &crypt_len);
